...
import { CurrencyPipe, NgIfContext } from '@angular/common';
...
export class PaymentsComponent {
  payments: {
    ...
    { id: 'P001', patientName: 'John Doe', date: '2024-12-01', amount: 150, currency: 'USD', status: 'Paid', method: 'Credit Card' },
    { id: 'P002', patientName: 'Jane Smith', date: '2024-11-01', amount: 200, currency: 'EUR', status: 'Pending', method: 'Insurance' },
    { id: 'P003', patientName: 'Alex John', date: '2024-12-01', amount: 100, currency: 'EUR', status: 'Overdue', method: 'Cash' }
  ];
  ...
  noPayments: TemplateRef<NgIfContext<boolean>> | null | undefined;

  get filteredPayments() {
    return this.payments.filter((payment) => {
      const matchesSearch =
        !this.searchQuery ||
        payment.patientName.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
  ...
  savePayment(): void {
    if (this.editing) {
      const index = this.payments.findIndex(p => p.id === this.newPayment.id);
      if (index !== -1) {
        this.payments[index] = { ...this.newPayment };
      }
    } else {
      this.addPayment();
    }
