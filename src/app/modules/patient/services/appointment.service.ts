...
export class AppointmentsService {
  private upcomingAppointmentsSource = new BehaviorSubject<{ date: string; time: string; reason: string }[]>([]);
  ...
  addAppointment(appointment: { date: string; time: string; reason: string }) {
    ...
    this.upcomingAppointmentsSource.next([...currentAppointments, appointment].sort((a, b) => {
  ...
  cancelAppointment(appointment: { date: string; time: string; reason: string }) {
